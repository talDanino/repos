...
scriptFlow();
///////////////////Function section
function scriptFlow(): void {
    let firstArgument: string;
    let secondArgument: string = "0";
    let shapeContentNumber: string;
    let shapeTypeNumer: string = input(1);
    let errorMessageString: string = errorMessage(1, shapeTypeNumer);
...
function input(questionNumber: number): string {
...
function errorMessage(errorType: number, valueToCheck: string): string {
    switch (errorType) {
        case 1:
...
function printShape(contentType: number, shapeType: number, firstArgument: number, secondArgument: number): void {
    let innerLoopEnd: number = 0;
    let outterLoopEnd: number = 0;
    let shapePerimeter: number = 0;
    let shapeArea: number = 0;
    let shapeTypeString: string = "";
    ...
    for (let i: number = 0; i <= outterLoopEnd; i++) {
        if (shapeType == 3) innerLoopEnd = i;
        let specialCounterForRhombusOption3: number = 1;
        for (let j: number = 0; j <= innerLoopEnd; j++) {
            switch (contentType) {
                case 1:
                    valToPrint = "*";
                    if (shapeType == 4) {
